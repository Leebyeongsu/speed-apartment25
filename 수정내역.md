...
### 문제 상황
- **증상**: 각 QR에 등록된 최대 3명의 이메일로만 알림이 발송되어야 하는데, 등록된 전체 QR의 모든 관리자에게 이메일이 발송되는 문제
- **영향**: QR 생성은 정상 작동하지만, 신청서 제출 시 다른 QR 관리자들도 알림을 받게 됨
- **Supabase**: 데이터베이스 자체는 문제 없음
...
고객 QR 스캔
    ↓
QR 데이터 조회 (qr_codes.emails) ✅
    ↓
❌ 조회한 데이터를 사용하지 않음!
    ↓
신청서 제출
    ↓
❌ admin_settings.emails 조회 (전체 관리자)
    ↓
❌ 모든 QR 관리자에게 발송
```

#### 2. 코드 레벨 원인
- **`sendNotificationsViaEdgeFunction()`** (Line 845-849): 항상 `admin_settings` 테이블만 조회
- **`sendEmailToAdmins()`** (Line 587-591): `localStorage`의 전역 이메일만 사용
- **QR 데이터 조회**: `setupCustomerMode()`에서 QR별 이메일을 조회하지만 로그만 출력하고 저장하지 않음

### 해결 방법

#### 1. 전역 변수 추가 (script.js Line 112-113)
```javascript
let currentQRRecipientEmails = []; // QR별 이메일 수신자 (고객 모드에서 설정)
let currentQRRecipientPhones = []; // QR별 전화번호 수신자 (고객 모드에서 설정)
```

#### 2. 고객 모드에서 QR 데이터 저장 (script.js Line 2001-2009)
```javascript
...
#### 3. sendNotificationsViaEdgeFunction() 수정 (script.js Line 845-874)
```javascript
...
#### 4. sendEmailToAdmins() 수정 (script.js Line 587-604)
```javascript
...
URL: ?mode=customer&qr_id=abc123
    ↓
currentQrId 설정: speed_apartment21_abc123
    ↓
QR 데이터 조회 → qr_codes.emails [QR별 이메일 3개]
    ↓
✅ currentQRRecipientEmails에 저장
    ↓
신청서 제출
    ↓
✅ currentQRRecipientEmails 우선 사용
    ↓
✅ 해당 QR의 이메일 3개로만 발송!
```
...
#### script.js
- **Line 112-113**: 전역 변수 추가 (`currentQRRecipientEmails`, `currentQRRecipientPhones`)
- **Line 2001-2009**: 고객 모드 QR 데이터 조회 시 이메일/전화번호 저장
- **Line 845-874**: `sendNotificationsViaEdgeFunction()` - QR 이메일 우선 사용 로직 추가
- **Line 587-604**: `sendEmailToAdmins()` - QR 이메일 우선 사용 로직 추가
...
### 문제 해결 완료
✅ QR별 이메일 격리 기능 정상 작동
✅ 각 QR에 등록된 3명에게만 이메일 발송
✅ 폴백 메커니즘 구현 (QR 이메일 없을 시 admin_settings 사용)
✅ 기존 기능 유지 (관리자 모드, 고객 모드)

### Git 커밋 정보
- **커밋 해시**: 4c2f65a
- **커밋 메시지**: "Fix: QR별 이메일 격리 기능 구현 - 각 QR에 등록된 이메일로만 알림 발송"
- **변경 파일**: script.js (72 insertions, 53 deletions)
- **푸시 완료**: https://github.com/Leebyeongsu/speed-apartment25.git

### 테스트 확인사항
- [x] QR 생성 시 이메일 3개 등록 가능
- [x] QR 스캔 후 신청서 제출 시 해당 QR 이메일로만 발송
- [x] 다른 QR 관리자에게 이메일 발송 안 됨
- [x] 폴백 메커니즘 정상 작동
- [x] 기존 관리자/고객 모드 기능 유지
...
1. **QR 생성 시**:
   - STEP 2 알림 설정에서 이메일 3개 등록 필수
   - 등록된 이메일은 `qr_codes` 테이블에 저장됨

2. **신청서 제출 시**:
   - 고객 모드에서 QR ID가 URL에 포함되어야 함 (`?mode=customer&qr_id=xxx`)
   - 콘솔에서 "✅ QR별 이메일 수신자 사용" 메시지 확인

3. **디버깅 시**:
   - 브라우저 콘솔에서 `currentQRRecipientEmails` 값 확인
   - "📧 QR별 이메일 수신자 저장" 로그 확인

---

*이 문서는 2025-09-28 긴급 수정 및 2025-10-31 QR별 이메일 격리 기능 구현 작업의 완전한 기록입니다.*